...
input1[0],input1[1],input1[2] = sac_am(np.array(data1["Eixo X"])),sac_am(np.array(data1["Eixo Y"])),sac_am(np.array(data1["Eixo Z"]))
if(len(sys.argv) > 2):
    file2 = sys.argv[2]+'.csv'
    data2 = pd.read_csv(file2)
    class_names = [sys.argv[1],sys.argv[2]]    
    input2[0],input2[1],input2[2] = sac_am(np.array(data2["Eixo X"])),sac_am(np.array(data2["Eixo Y"])),sac_am(np.array(data2["Eixo Z"]))

if(len(sys.argv) > 3):
    file3 = sys.argv[3]+'.csv'
    data3 = pd.read_csv(file3)
    class_names = ["VN","CF1","CF2"]
    input3[0],input3[1],input3[2] = sac_am(np.array(data3["Eixo X"])),sac_am(np.array(data3["Eixo Y"])),sac_am(np.array(data3["Eixo Z"]))

if(len(sys.argv) > 4):
    file4 = sys.argv[4]+'.csv'
    data4 = pd.read_csv(file4)
    class_names = [sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4]]
    input4[0],input4[1],input4[2] = sac_am(np.array(data4["Eixo X"])),sac_am(np.array(data4["Eixo Y"])),sac_am(np.array(data4["Eixo Z"]))
...
#usar esse
#classifier = svm.SVC(gamma=2, C=1).fit(X_train, y_train)
...
titles_options = [("Confusion matrix, without normalization", None),
                  ("Matriz de Confusão com Sinais Brutos", 'true')]
...
knn = pickle.load(open("KNN_final.sav", 'rb'))
res = knn.predict(y)
print(res)
